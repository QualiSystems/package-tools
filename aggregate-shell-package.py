...
def write_version_file(pkg_path):
    # set cururent version to versioln.txt file
...
def write_setup_file(pkg_path):
    ...
    pkg_name_str = re.sub('-', ' ', INSTALLATION_PACKAGE_NAME)

    SETUP_TEMPLATE = re.sub('\$PACKAGE_NAME', pkg_name_str)
    SETUP_TEMPLATE = re.sub()
